(function($) {
	...
	$(function() {
		...
		$('#summernote').summernote({
			callbacks: {
			  onChange: function(contents, $editable) {
				  $("#panelViewNotificacion").html(contents);
		...
		///////////////////////////
		$('.tdv_tables').DataTable({
            "language": {
                "url": "languajes/Spanish.json"
            }
		});

		var tblMiembros = $('#tdv_tbl_Miembros').DataTable({
			"buttons": [
				'copy', 'excel', 'pdf'
			],
			"language": {
			...
			"serverSide": true,
			"ajax":{
				url :"server/miembros/getConstituyentes.php", // json datasource
				type: "post",  // type of method  ,GET/POST/DELETE
				error: function(){
					$("#tdv_tbl_Miembros_processing").css("display","none");
				}
			},
			"columns": [
				{ 
			...
			"columnDefs": [ {
				"targets": -1,
				"data": null,
				"defaultContent": '<button type="button" class="btn btn-inverse-warning btn-icon btnEnviarFelicitaciones"> <i class="mdi mdi-send"></i> </button> <button type="button" class="btn btn-inverse-success btn-icon btnEditarMiembro" data-toggle="modal" data-target="#modalMiembrosForm"> <i class="mdi mdi-grease-pencil"></i> </button> <button type="button" class="btn btn-inverse-danger btn-icon btnEliminarMiembro" data-toggle="modal" data-target="#modalDelete"> <i class="mdi mdi-minus-box"></i> </button>'
			} ]
		}); 
		$('#tdv_tbl_Miembros tbody').on( 'click', '.btnEnviarFelicitaciones', function () {
			var data = tblMiembros.row( $(this).parents('tr') ).data();
			TDV.enviarFelicitacion(data[7]);
		});
		$('#tdv_tbl_Miembros tbody').on( 'click', '.btnEditarMiembro', function () {
			var data = tblMiembros.row( $(this).parents('tr') ).data();
			TDV.getMemberOne22(data[7], "MIEMBROS");
		});
		$('#tdv_tbl_Miembros tbody').on( 'click', '.btnEliminarMiembro', function () {
			var data = tblMiembros.row( $(this).parents('tr') ).data();
			$('#btnDeleteReg').attr('data-table', 'miembros'); 
			$('#btnDeleteReg').attr('data-rel', data[7]);
		});

		var tblMiembros2 = $('#tdv_tbl_Miembros2').DataTable({
			"language": {
                "url": "languajes/Spanish.json"
            },
			"bProcessing": true,
			"serverSide": true,
			"ajax":{
				url :"server/miembros/getConstituyentes2.php", // json datasource
				type: "post",  // type of method  ,GET/POST/DELETE
				error: function(){
					$("#tdv_tbl_Miembros2_processing").css("display","none");
				}
			},
			columns: [
				{ 
			...
			"columnDefs": [ {
				"targets": -1,
				"data": null,
				"defaultContent": '<button type="button" class="btn btn-inverse-warning btn-icon btnEnviarLink"> <i class="mdi mdi-email"></i> </button> <button type="button" class="btn btn-inverse-info btn-icon btnCrearLink" data-toggle="modal" data-target="#modalCrearLink"> <i class="mdi mdi-link"></i> </button> <button type="button" class="btn btn-inverse-warning btn-icon btnEnviarFelicitaciones"> <i class="mdi mdi-send"></i> </button> <button type="button" class="btn btn-inverse-success btn-icon btnEditarMiembro" data-toggle="modal" data-target="#modalMiembrosForm"> <i class="mdi mdi-grease-pencil"></i> </button> <button type="button" class="btn btn-inverse-danger btn-icon btnEliminarMiembro" data-toggle="modal" data-target="#modalDelete"> <i class="mdi mdi-minus-box"></i> </button>'
			} ]
		}); 
		$('#tdv_tbl_Miembros2 tbody').on( 'click', '.btnEnviarFelicitaciones', function () {
			var data = tblMiembros2.row( $(this).parents('tr') ).data();
			TDV.enviarFelicitacion(data[7]);
		});
		$('#tdv_tbl_Miembros2 tbody').on( 'click', '.btnEditarMiembro', function () {
			var data = tblMiembros2.row( $(this).parents('tr') ).data();
			TDV.getMemberOne2(data[7], "MIEMBROS");
		});
		$('#tdv_tbl_Miembros2 tbody').on( 'click', '.btnEliminarMiembro', function () {
			var data = tblMiembros2.row( $(this).parents('tr') ).data();
			$('#btnDeleteReg').attr('data-table', 'miembros'); 
			$('#btnDeleteReg').attr('data-rel', data[7]);
		});
		$('#tdv_tbl_Miembros2 tbody').on( 'click', '.btnCrearLink', function () {
			var data = tblMiembros2.row( $(this).parents('tr') ).data();
			$('#linkMiembroUpdate').val('https://app.tabernaculodevida.es?idConstituyente='+data[7]);
		});
		$('#tdv_tbl_Miembros2 tbody').on( 'click', '.btnEnviarLink', function () {
			var data = tblMiembros2.row( $(this).parents('tr') ).data();
			TDV.enviarLinkMembresia(data[7]);
		});

		var tblFamilias = $('#tdv_tbl_M_Familias').DataTable({
			"language": {
                "url": "languajes/Spanish.json"
            },
			"bProcessing": true,
			"serverSide": true,
			"ajax":{
				url :"server/miembros/getFamilias.php", // json datasource
				type: "post",  // type of method  ,GET/POST/DELETE
				error: function(){
					$("#tdv_tbl_M_Familias_processing").css("display","none");
				}
			},
			columns: [
				{ title: "FAMILIA" },
			...
			"columnDefs": [ {
				"targets": -1,
				"data": null,
				"defaultContent": '<button type="button" class="btn btn-inverse-success btn-icon btnEditarMiembro" data-toggle="modal" data-target="#modalMiembrosFamiliasForm"> <i class="mdi mdi-grease-pencil"></i> </button> <button type="button" class="btn btn-inverse-danger btn-icon btnEliminarMiembro" data-toggle="modal" data-target="#modalDelete"> <i class="mdi mdi-minus-box"></i> </button>'
			} ]
		}); 
		$('#tdv_tbl_M_Familias tbody').on( 'click', '.btnEditarMiembro', function () {
			var data = tblFamilias.row( $(this).parents('tr') ).data();
			TDV.getFamiliaOne(data[2]);
		});
		$('#tdv_tbl_M_Familias tbody').on( 'click', '.btnEliminarMiembro', function () {
			var data = tblFamilias.row( $(this).parents('tr') ).data();
	
			$('#btnDeleteReg').attr('data-table', 'familias'); 
			$('#btnDeleteReg').attr('data-rel', data[2]);
		});
