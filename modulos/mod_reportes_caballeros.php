<?php  
    ...
    $DEPARTAMENTO = "AND idDepartamento = 7";
    ...
    if(!$TDV->segurity($modulo, $TDV->getArrayModules())){
        ...
        if( ($FECHAS == "") && ($BAUTI == "") && ($ESSANTO == "") && ($DEPARTAMENTO == "") && ($TIPO == "") ){
            ...
            $rs02 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 7 ");
            $numDAMAS = mysqli_num_rows($rs02);

            $rs05 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 7 AND bautizado = 'on' ");
            $numBAUTIZADOS = mysqli_num_rows($rs05);

            $rs06 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 7 AND espirituSanto = 'on' ");
            $numESPIRITUSANTO = mysqli_num_rows($rs06);
        }else{
            $SQL = "SELECT * FROM tdv_miembros WHERE idDepartamento = 7 ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$TIPO." ORDER BY fechaVisita DESC";        
            ...
            $rs02 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 7 ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$TIPO);
            $numDAMAS = mysqli_num_rows($rs02);

            $rs05 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 7 AND bautizado = 'on' ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$TIPO);
            $numBAUTIZADOS = mysqli_num_rows($rs05);

            $rs06 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 7 AND espirituSanto = 'on' ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$TIPO);
            $numESPIRITUSANTO = mysqli_num_rows($rs06);
        }
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        ...
                        <input type="hidden" id="txt_idDepartamento" name="txt_idDepartamento" value="7">
