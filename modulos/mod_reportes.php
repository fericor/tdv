<?php  
    ...
    if(($F_INI != "") && ($F_FIN != "")){
        $FECHAS  = "date(fechaVisita) BETWEEN TIMESTAMP('{$F_INI}') AND TIMESTAMP('{$F_FIN}')";
    }else if(($F_INI != "") && ($F_FIN == "")){
        $FECHAS  = "date(fecha) >= TIMESTAMP('{$F_INI}', '%y/%m/%d')";
    }else{
        $FECHAS  = "";
    }
    ...
    $DEPARTAMENTO = $_POST["txt_idDepartamento"] != "" ? "AND idDepartamento = '{$_POST["txt_idDepartamento"]}'" : '';
    $TIPO         = $_POST["txt_idTipoMiembro"] != "" ? "AND idTipoMiembro = '{$_POST["txt_idTipoMiembro"]}'" : '';

    $BAUTIZADOII     = $BAUTI == "on" ? '<label class="badge badge-info tdv_click">SI</label>' : '<label class="badge badge-danger tdv_click">NO</label>';
    $ESPIRITUSANTOII = $ESSANTO == "on" ? '<label class="badge badge-info tdv_click">SI</label>' : '<label class="badge badge-danger tdv_click">NO</label>';
    ...
    if(!$TDV->segurity($modulo, $TDV->getArrayModules())){
        ...
        if( ($FECHAS == "") && ($BAUTI == "") && ($ESSANTO == "") && ($DEPARTAMENTO == "") && ($TIPO == "") ){
            ...
            $rs01 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 7 ");
            ...
            $rs03 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 4 ");
            $numJOVENES = mysqli_num_rows($rs03);

            $rs04 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 5 ");
            $numESCUELADOMINICAL = mysqli_num_rows($rs04);

            $rs05 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE bautizado = 'on' ");
            $numBAUTIZADOS = mysqli_num_rows($rs05);

            $rs06 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE espirituSanto = 'on' ");
            $numESPIRITUSANTO = mysqli_num_rows($rs06);
        }else{
            $SQL = "SELECT * FROM tdv_miembros WHERE ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$DEPARTAMENTO." ".$TIPO." ORDER BY fechaVisita DESC";        
            $INFORMEBUSQUEDA = '<b>Fechas: </b> <i>'.$D_INI->format('d/m/Y').'</i> y <i>'.$D_FIN->format('d/m/Y').'</i> <br>';
            if( ($BAUTI == "") || ($ESSANTO == "") ){
                $INFORMEBUSQUEDA .= '';
            }else{
                $INFORMEBUSQUEDA .= '<b>Bautizados: </b> <i>'.$BAUTIZADOII.'</i>  -  <b>Espíritu Santo: </b> <i>'.$ESPIRITUSANTOII.'</i> <hr>';
            }
            
            $rs01 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 7 AND ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$DEPARTAMENTO." ".$TIPO);
            $numCABALLEROS = mysqli_num_rows($rs01);

            $rs02 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 11 AND ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$DEPARTAMENTO." ".$TIPO);
            $numDAMAS = mysqli_num_rows($rs02);

            $rs03 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 4 AND ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$DEPARTAMENTO." ".$TIPO);
            $numJOVENES = mysqli_num_rows($rs03);

            $rs04 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE idDepartamento = 5 AND ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$DEPARTAMENTO." ".$TIPO);
            $numESCUELADOMINICAL = mysqli_num_rows($rs04);

            $rs05 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE bautizado = 'on' AND ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$DEPARTAMENTO." ".$TIPO);
            $numBAUTIZADOS = mysqli_num_rows($rs05);

            $rs06 = $TDV->conn->query("SELECT * FROM tdv_miembros WHERE espirituSanto = 'on' AND ".$FECHAS." ".$BAUTI." ".$ESSANTO." ".$DEPARTAMENTO." ".$TIPO);
            $numESPIRITUSANTO = mysqli_num_rows($rs06);
        }
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col">
                            <span class="pull-left">Departamentos:</span>
                            <select class="form-control" id="txt_idDepartamento" name="txt_idDepartamento" style="width: 100%;height: 45px;">
                                <?=$DEPARTAMENTOS?>
                            </select>
                        </div>
                        <div class="col">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col">
                        ...
                        <div class="row">
                            <div class="col">
                                <b>Caballeros: </b> <i><?=$numCABALLEROS?></i><br>
                                <b>Damas: </b> <i><?=$numDAMAS?></i>
                            </div>
                            <div class="col">
                                <b>Jóvenes: </b> <i><?=$numJOVENES?></i> <br>
                                <b>Escuela Dominical: </b> <i><?=$numESCUELADOMINICAL?></i>
                            </div>
                        </div>
                         <hr>
                        <div class="row">
